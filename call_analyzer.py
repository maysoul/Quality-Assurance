...
try:
    ...
    while transcript.status != aai.TranscriptStatus.completed:
        ...
        if transcript.status == aai.TranscriptStatus.completed:
            ...
            print(f"Informal Summary from AssemblyAI:\n{informal_summary}") # Print the original summary
            ...
            print(f"\nFormal, Structured Summary:\n{formal_summary}") # Print the transformed summary


            # --- Basic Agent Performance Evaluation (Keyword-based) --- (Keep this part as is for now)
            ...
            transcript_lower = transcript.text.lower() # Convert to lowercase for case-insensitive matching

            for word in positive_keywords:
                positive_count += transcript_lower.count(word)
            for word in negative_keywords:
                negative_count += transcription_lower.count(word)

            performance_score = positive_count - negative_count

            print("\n--- Agent Performance Evaluation (Basic) ---")
            print(f"Positive keyword count: {positive_count}")
            print(f"Negative keyword count: {negative_count}")
            print(f"Performance Score: {performance_score}") # Higher score is better (in this simple model)

            if performance_score > 0:
                print("Overall Performance: Potentially Positive")
            elif performance_score < 0:
                print("Overall Performance: Potentially Negative")
            else:
                print("Overall Performance: Neutral or Mixed")
